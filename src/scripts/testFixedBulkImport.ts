// Test script for the fixed bulk import parsing
import { parseBulkImportData } from './enhancedBulkImport';

// Test data with correct format (no extra blank lines)
...
const github = parsedData.find((w: any) => w.title === 'GitHub');
console.log(`GitHub category: ${github?.category} (expected: dev-tools) - ${github?.category === 'dev-tools' ? 'PASS' : 'FAIL'}`);
...
console.log(`\nTotal websites parsed: ${parsedData.length} (expected: 11) - ${parsedData.length === 11 ? 'PASS' : 'FAIL'}`);